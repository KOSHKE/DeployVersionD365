...
def runOperation(path: str, parameters):
    ...
    if (path == ''):
        window = PySimpleGUI.Window('Path is not defined', layouts.getLayout('ErrorWindow', 'Path is not defined'), background_color='red')
        window.read()
        return False
    ...
    processHotfixInstallationInfo(path + '/HotfixInstallationInfo.xml', parameters)
    return True
